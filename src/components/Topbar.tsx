...
export default function Topbar() {
  ...
  // Derive filtered results from query
      ...
      <div className="flex items-center gap-3">
        ...
        <div ref={searchRef} className="relative hidden max-w-xs md:block">
          <div className="relative">
            ...
            <input
              ...
              className="auth-input w-full rounded-full py-2 pl-10 pr-4 text-[13px]"
              style={{
                ...
                results.map((r) => (
                  <button
                    ...
                    onClick={() => {
                      setQuery("");
                      setShowResults(false);
                      router.push(`/dashboard/resources?play=${r.id}`);
                    }}
        ...
        <div ref={soundRef} className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded-full px-3.5 py-1.5 text-[11px] font-semibold transition-all duration-150 hover:brightness-95"
            style={{
          ...
          >
            ...
            {playing ? "Playing" : "Play Nature Sound"}
          </button>

          {/* Volume panel */}
          {showVolume && playing && (
            <div
              className="absolute right-0 top-full mt-2 w-52 rounded-xl p-3"
              style={{
            ...
            >
              <div className="mb-2.5 flex items-center justify-between">
                <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#8DBFA5" }}>Volume</span>
                <span className="text-[11px] font-bold tabular-nums" style={{ color: "#4A9474" }}>{Math.round(volume)}%</span>
              </div>
              <input
                type="range"
                min={0}
              ...
              />
              <button
                onClick={() => { stop(); setShowVolume(false); }}
                className="mt-3 w-full rounded-lg py-1.5 text-[11px] font-semibold transition-all duration-150 hover:brightness-95"
                style={{
                  backgroundColor: "rgba(200,230,208,0.15)",
                  color: "#6B9E85",
                  border: "1px solid rgba(200,230,208,0.3)",
                }}
              >
                Stop sound
              </button>
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[11px] font-medium transition-all duration-150 hover:brightness-95"
          style={{
            color: "#6B9E85",
